...
export function App() {
  return (
    <PrendaThemeProvider>
      ...
      <Button variant="stroke" color="prendaBlue">Stroke</Button>
    </PrendaThemeProvider>
  );
}
```
...
- Palette extensions: `prendaBlue`, `prendaGrey`, `prendaRed`, `prendaYellow`, `prendaGreen`, `prendaMagenta` (typed for `color` props on MUI components like `Button`).
...
```ts
import { createPrendaTheme, PrendaThemeProvider } from "@prenda-school/prenda-ds";
